...
def get_invoices(db: Session = Depends(get_db), access = Depends(get_access_user)):
    invoice = db.query(Invoice).all()
    return invoice


@router.post("/", status_code=status.HTTP_201_CREATED, response_model=InvoiceResponse)
def create_invoice(invoice: InvoiceBase, db: Session = Depends(get_db), access = Depends(get_access_user)):
